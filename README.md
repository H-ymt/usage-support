...
## LINE LIFF Development Setup

For LINE LIFF authentication development, you need to expose your local server to the internet using ngrok:

### 1. Install ngrok and authenticate

```bash
# ngrokアカウント作成後、authtokenを設定
...
- ngrok tunnels (両方のポートを公開)

### 3. ngrok URLを環境変数に設定

ngrok起動後、表示されるHTTPS URLを環境変数に設定:

```bash
# apps/web/.env
VITE_SERVER_URL=https://your-server-url.ngrok-free.app

# apps/server/.env
BETTER_AUTH_URL=https://your-server-url.ngrok-free.app
CORS_ORIGIN=https://your-web-url.ngrok-free.app
```

### 4. LINE Developers設定

1. [LINE Developers Console](https://developers.line.biz/) でLIFFアプリを作成
2. Endpoint URLに ngrok の Web URL を設定: `https://your-web-url.ngrok-free.app`
3. LIFF IDを取得して `apps/web/.env` の `VITE_LIFF_ID` に設定

### ngrok設定ファイル

`ngrok.yml` で設定をカスタマイズ可能:
- 固定ドメイン (有料プラン)
- カスタムパス
- Basic認証など
...
- `pnpm run dev:liff`: Start web + server + ngrok (for LINE LIFF development)
- `pnpm run dev:web`: Start only the web application
- `pnpm run dev:server`: Start only the server
- `pnpm run ngrok:start`: Start ngrok tunnels only
...
- `pnpm run supabase:types`: Generate TypeScript types from schema
